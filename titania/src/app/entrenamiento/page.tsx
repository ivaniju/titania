...
export default function EntrenamientoPage() {
  ...
  const { todayWorkout, completeWorkout, selectedRoutineId } = useTitanStore();
  if (!ready) return null;
  const routineName = ROUTINES.find((r) => r.id === selectedRoutineId)?.name ?? "";
  ...
  return (
    <div className="flex min-h-dvh flex-col gap-4 px-5 pb-[110px]">
      ...
      <NavCard
        ...
        icon="🗂️"
        title="Ver rutinas"
        subtitle={routineName ? `Activa: ${routineName}` : "Elige tu programa"}
      />
